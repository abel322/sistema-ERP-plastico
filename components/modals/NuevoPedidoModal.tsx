...
export function NuevoPedidoModal({ isOpen, onClose, onSuccess }: NuevoPedidoModalProps) {
    ...
    const handleClienteChange = (clienteId: string) => {
        ...
        // Asignar unidad automáticamente según tipo de producto
        let unidadAuto = '';
        if (cliente?.tipoProducto === 'Bolsa') {
            unidadAuto = 'Unidades';
        } else if (cliente?.tipoProducto === 'Bobina') {
            unidadAuto = 'Kilogramos';
        } else {
            unidadAuto = cliente?.unidadVenta || '';
        }
                ...
                >
                                        ...
                                        {clienteSeleccionado && (
                                            ...
                                            >
                                                <div className="flex items-center gap-2 mb-3 text-blue-900">
                                                    ...
                                                    <h4 className="text-sm font-bold uppercase tracking-tight">Información del Producto</h4>
                                                </div>
                                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 text-sm">
                                                    <div className="flex flex-col">
                                                        <span className="text-blue-600 text-[10px] font-bold uppercase">Tipo</span>
                                                        <span className="text-blue-900 font-semibold">{clienteSeleccionado.tipoProducto}</span>
                                                    </div>
                                                    <div className="flex flex-col">
                                                        <span className="text-blue-600 text-[10px] font-bold uppercase">Unidad</span>
                                                        <span className="text-blue-900 font-semibold">{clienteSeleccionado.unidadVenta}</span>
                                                    </div>
                                                    {clienteSeleccionado?.ancho && (
                                                        <div className="flex flex-col">
                                                            <span className="text-blue-600 text-[10px] font-bold uppercase">Medidas</span>
                                                            <span className="text-blue-900 font-semibold">
                                                                {clienteSeleccionado.ancho}x{clienteSeleccionado.largo}x{clienteSeleccionado.calibre} cm/μm
                                                            </span>
                                                        </div>
                                                    )}
                                                </div>
                    ...
                    <div className="p-5 border-t border-gray-100 bg-gray-50 flex justify-end gap-3">
                        ...
                        <button
                            ...
                            disabled={loading || fetchingClientes || !formData.clienteId}
