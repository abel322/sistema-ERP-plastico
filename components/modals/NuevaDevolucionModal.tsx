...
export function NuevaDevolucionModal({ isOpen, onClose, onSuccess }: NuevaDevolucionModalProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            setSaving(true);
            // Reutilizamos el endpoint para crear ProductoTerminado, pero ya lleva el estado "Defectuoso"
            ...
            if (res.ok) {
                ...
                alert(err.error || 'Error al guardar el producto defectuoso');
            }
        } catch (error) {
            console.error(error);
    ...
    return (
        <AnimatePresence>
            <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
                ...
                >
                    {/* Header */}
                    <div className="flex items-center justify-between p-6 bg-gradient-to-r from-red-600 to-orange-600 text-white">
                        <div className="flex items-center gap-3">
                            <div className="p-2 bg-white/20 rounded-lg">
                                <AlertTriangle className="w-6 h-6" />
                            </div>
                            <div>
                                <h2 className="text-xl font-bold">Registrar Devolución Manual</h2>
                                <p className="text-red-100 text-sm">Ingreso manual de producto defectuoso a bodega</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
                    ...
                    <div className="p-5 border-t border-gray-100 bg-gray-50/50 flex justify-end gap-3">
                        ...
                        <button
                            ...
                            className="px-6 py-2.5 bg-red-600 hover:bg-red-700 text-white text-sm font-semibold rounded-xl shadow-sm hover:shadow active:scale-95 transition-all flex items-center gap-2 disabled:opacity-50"
                        >
                            {saving ? <LoadingSpinner /> : <><Save className="w-4 h-4" /> Registrar Devolución</>}
                        </button>
                    </div>
                </motion.div>
            </div>
