...
export default function PedidosPage() {
  ...
  return (
    <div className="p-4 sm:p-6 md:p-8 bg-slate-50 dark:bg-slate-950 min-h-screen transition-colors duration-300">
      {/* Header Area */}
      <div className="mb-6 md:mb-8">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 md:gap-6 bg-white dark:bg-slate-900 p-4 sm:p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 transition-colors">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 sm:w-14 sm:h-14 bg-blue-600 rounded-2xl flex items-center justify-center text-white shadow-lg shadow-blue-200 dark:shadow-none flex-shrink-0">
              <Package className="w-6 h-6 sm:w-8 sm:h-8" />
            </div>
            <div>
              <h1 className="text-xl sm:text-2xl md:text-3xl font-black text-slate-900 dark:text-white leading-tight">Pedidos</h1>
              <div className="flex flex-wrap items-center gap-2 mt-1">
                ...
                <span className="w-1 h-1 bg-slate-300 dark:bg-slate-700 rounded-full hidden sm:inline-block" />
          ...
          <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto justify-end">
            <Link href="/pedidos/historial" className="flex-1 sm:flex-none">
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="w-full sm:w-auto flex items-center justify-center gap-2 rounded-xl bg-slate-100 dark:bg-slate-800 px-4 sm:px-5 py-2.5 font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all shadow-sm text-xs sm:text-sm"
              >
            ...
            {isAdmin && (
              <motion.button
                ...
                className="flex-1 sm:flex-none flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 sm:px-5 py-2.5 font-bold text-white shadow-lg transition-all hover:bg-blue-700 active:scale-95 text-xs sm:text-sm"
              >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800 p-4 sm:p-6">
        ...
        <div className="mb-6 flex flex-col xl:flex-row xl:items-center justify-between gap-4">
          <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 flex-1 w-full">
            <div className="relative flex-1 w-full min-w-0 sm:min-w-[240px] md:min-w-[300px]">
            ...
            <div className="flex flex-wrap sm:flex-nowrap items-center gap-2 bg-slate-50 dark:bg-slate-800 p-1.5 rounded-xl border border-slate-200 dark:border-slate-700">
              <div className="px-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest border-r border-slate-200 dark:border-slate-700 hidden sm:block">Filtros</div>
              <select
                value={estado}
                onChange={(e) => {
                ...
                className="bg-transparent px-2.5 py-1.5 text-xs font-bold text-slate-600 dark:text-slate-300 focus:outline-none cursor-pointer flex-1 sm:flex-none"
              >
              ...
              <div className="w-px h-4 bg-slate-200 dark:border-slate-700 hidden sm:block" />
              <select
                value={prioridad}
                onChange={(e) => {
        ...
        {/* Vista Móvil: Tarjetas (block md:hidden) */}
        <div className="block md:hidden space-y-4">
          {pedidos?.map((pedido) => {
            const saldo = (pedido?.cantidadSolicitada || 0) - (pedido?.cantidadProducida || 0);
            return (
              <div key={pedido?.id} className="bg-slate-50 dark:bg-slate-800/60 p-4 rounded-xl border border-slate-200 dark:border-slate-800 space-y-3 shadow-sm">
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <button
                      onClick={() => router.push(`/clientes/${pedido?.cliente?.id}`)}
                      className="font-bold text-slate-900 dark:text-white hover:text-blue-600 dark:hover:text-blue-400 transition-colors text-sm text-left"
                    >
                    ...
                    <div className="text-xs font-semibold text-slate-700 dark:text-slate-300 mt-0.5">
                      ...
                      {pedido?.productoCliente?.codigoProducto && (
                        <span className="ml-1 text-[10px] text-slate-400 font-bold uppercase tracking-widest">({pedido.productoCliente.codigoProducto})</span>
                      )}
                    </div>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs pt-2.5 border-t border-slate-200/60 dark:border-slate-700/60">
                  <div>
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Cantidad</span>
                    <span className="font-bold text-slate-900 dark:text-white text-xs">
                      {pedido?.cantidadSolicitada?.toLocaleString() || 0} {pedido?.unidad || ''}
                    </span>
                    <span className="text-[10px] text-slate-400 block mt-0.5">Saldo: {saldo.toLocaleString()}</span>
                  </div>
                  <div>
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-wider block">Fechas</span>
                    <span className="font-bold text-blue-600 dark:text-blue-400 text-xs block">
                      Entrega: {pedido?.fechaEntrega ? format(new Date(pedido.fechaEntrega), 'dd/MM/yyyy') : '-'}
                    </span>
                    <span className="text-[10px] text-slate-400 block mt-0.5">
                      Pedido: {pedido?.fechaPedido ? format(new Date(pedido.fechaPedido), 'dd/MM/yyyy') : '-'}
                    </span>
                  </div>
                </div>
