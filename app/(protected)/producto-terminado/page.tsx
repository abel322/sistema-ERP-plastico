                    ...
                    <table className="w-full border-separate border-spacing-y-3">
                      <thead>
                        <tr className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">
                          <th className="px-6 py-2 text-left">Tipo & Producto</th>
                          <th className="px-6 py-2 text-left">Cliente</th>
                          <th className="px-6 py-2 text-center">Especificaciones</th>
                      ...
                      <tbody>
                        {sobrantes.map((sobrante) => (
                          <motion.tr
                            ...
                            className="group bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-all duration-300 rounded-2xl shadow-sm hover:shadow-md border border-slate-100 dark:border-slate-800"
                          >
                            <td className="px-6 py-4 first:rounded-l-2xl border-y border-l border-slate-100 dark:border-slate-800 group-hover:border-slate-200 dark:group-hover:border-slate-700">
                              <div className="flex items-center gap-4">
                                ...
                                <div className="flex flex-col">
                                  <span className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-tight">{sobrante.tipo}</span>
                                  <div className="flex items-center gap-2 mt-0.5">
                                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{sobrante.producto?.nombreProducto || 'Sin especificar'}</span>
                                    <span className="w-1 h-1 bg-slate-200 dark:bg-slate-700 rounded-full" />
                                    <span className="text-[10px] font-medium text-slate-400 italic">{new Date(sobrante.fecha).toLocaleDateString('es-VE')}</span>
                                  </div>
                                </div>
                              </div>
                            </td>
                            <td className="px-6 py-4 border-y border-slate-100 dark:border-slate-800 group-hover:border-slate-200 dark:group-hover:border-slate-700">
                              <div className="flex items-center gap-2">
                                <div className="w-7 h-7 bg-blue-50 dark:bg-blue-900/30 rounded-lg flex items-center justify-center text-blue-600">
                                  <CheckCircle className="w-3.5 h-3.5" />
                                </div>
                                <span className="text-xs font-bold text-slate-600 dark:text-slate-300 uppercase tracking-wide truncate max-w-[150px]">
                                  {sobrante.cliente?.nombre || 'General'}
                                </span>
                              </div>
                            </td>
                            <td className="px-6 py-4 border-y border-slate-100 dark:border-slate-800 group-hover:border-slate-200 dark:group-hover:border-slate-700">
                              <div className="flex flex-wrap items-center justify-center gap-2">
                                {[
                                  { l: 'A', v: sobrante.ancho },
                                  { l: 'L', v: sobrante.largo },
                                  { l: 'C', v: sobrante.calibre },
                                  { l: 'F', v: sobrante.fuelles },
                                  { l: 'AT', v: sobrante.anchoTroquel },
                                  { l: 'LT', v: sobrante.largoTroquel }
                                ].filter(s => s.v).map((s, i) => (
                                  <span key={i} className="px-2 py-1 bg-slate-50 dark:bg-slate-800 text-[10px] font-black text-slate-500 border border-slate-100 dark:border-slate-700 rounded-lg">
                                    <span className="text-[8px] opacity-50 mr-1">{s.l}:</span>{s.v}
                                  </span>
                                ))}
                                {!sobrante.ancho && !sobrante.largo && !sobrante.calibre && (
                                  <span className="text-[10px] text-slate-300 italic">No especificado</span>
                                )}
                              </div>
                            </td>
                            <td className="px-6 py-4 text-right border-y border-slate-100 dark:border-slate-800 group-hover:border-slate-200 dark:group-hover:border-slate-700">
                              <div className="flex flex-col items-end">
                                <span className="text-base font-black text-slate-900 dark:text-white tracking-tighter">
                                  ...
                                  <span className="text-[10px] text-slate-400 font-bold ml-1 uppercase">{sobrante.unidad === 'Kilogramos' ? 'Kg' : 'Und'}</span>
                                </span>
                                <span className="text-[8px] font-black text-emerald-600 dark:text-emerald-400 uppercase tracking-widest mt-0.5">Disponible</span>
                              </div>
                            </td>
                            <td className="px-6 py-4 last:rounded-r-2xl border-y border-r border-slate-100 dark:border-slate-800 group-hover:border-slate-200 dark:group-hover:border-slate-700 text-right">
                              <div className="flex items-center justify-end gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <button
                                  onClick={() => handleEliminarSobrante(sobrante.id)}
