...
import { Plus, Search, Edit, Trash2, X, UserPlus, Phone, Mail, Package, Building2, Sparkles, ChevronDown, ChevronUp, Droplets, Grid, Settings2, Palette, Loader2 } from 'lucide-react';
                  ...
                  <div className="flex-1 overflow-y-auto p-5 sm:p-8">
                    ...
                    {activeTab === 'especificaciones' && (
                      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                        ...
                        {/* Extras por Tipo de Producto */}
                        {formData.tipoProducto === 'Bolsa' && (
                          <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                            <h4 className="text-sm font-bold text-gray-900 border-b border-gray-200 pb-2 mb-4">Estructura de la Bolsa</h4>
                        ...
                        <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100">
                          ...
                          <div className="grid grid-cols-2 gap-4 max-w-lg">
                            <div>
                              ...
                              {(formData.ancho || formData.anchoBobina || formData.calibre) && (
                                <div className="mt-2 text-xs text-gray-600">
                                  {formData.tipoBobinaCliente === 'Lamina' && formData.tipoSellado === 'Inferior' && formData.esBolsaPego ? (
                                    <>
                                      <span className="font-semibold">Ancho:</span> {
                                        formData.ancho && formData.anchoFuelle && formData.anchoSolapa
                                          ? ((parseFloat(formData.ancho) * 2) + (parseFloat(formData.anchoFuelle) * 2) + parseFloat(formData.anchoSolapa)).toFixed(2)
                                          : '-'
                                      } cm, <span className="font-semibold">Calibre:</span> {formData.calibre || '-'} µ
                                    </>
                                  ) : formData.tipoBobinaCliente === 'Lamina' && formData.tipoSellado === 'Lateral' ? (
                                    <>
                                      <span className="font-semibold">Ancho:</span> {formData.largo ? (parseFloat(formData.largo) * 2).toFixed(2) : '-'} cm, <span className="font-semibold">Calibre:</span> {formData.calibre ? (parseFloat(formData.calibre) / 2).toFixed(2) : '-'} µ
                                    </>
                                  ) : formData.tipoBobinaCliente === 'Manga' && formData.tipoSellado === 'Lateral' ? (
                                    <>
                                      <span className="font-semibold">Ancho:</span> {formData.largo || '-'} cm, <span className="font-semibold">Calibre:</span> {formData.calibre || '-'} µ
                                    </>
                                  ) : (
                                    <>
                                      <span className="font-semibold">Ancho:</span> {formData.tipoProducto === 'Bobina' ? formData.anchoBobina || '-' : formData.ancho || '-'} cm, <span className="font-semibold">Calibre:</span> {formData.calibre || '-'} µ
                                    </>
                                  )}
                                </div>
                              )}
                        ...
                        <div className="space-y-4 pt-2">
                          <h4 className="text-sm font-bold text-gray-900 border-b border-gray-100 pb-2">Acabados Especiales</h4>
                          <div className="flex flex-wrap items-center gap-6">
                            ...
                            <label className="flex items-center gap-2 cursor-pointer pt-4 group">
                              ...
                              <span className="text-sm font-medium text-gray-700 group-hover:text-purple-700">Lleva Perforación</span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer pt-4 group">
                        ...
                        <div className="p-6 bg-gradient-to-r from-pink-50 to-rose-50 rounded-2xl border border-pink-100 mb-6">
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                            <div><label className="text-xs font-bold tracking-wide text-pink-800 uppercase mb-2 block">Cilindro Base</label>
                              <input type="text" placeholder="ID del cilindro" value={formData.cilindro} onChange={e => setFormData({ ...formData, cilindro: e.target.value })} className="w-full bg-white border border-pink-200 px-4 py-2.5 rounded-xl text-sm font-medium outline-none focus:border-pink-500 focus:ring-4 focus:ring-pink-500/10" /></div>
                            <div><label className="text-xs font-bold tracking-wide text-pink-800 uppercase mb-2 block">Tratador (Intensidad)</label>
                              <input type="number" step="0.01" placeholder="Nivel" value={formData.intensidadTratador} onChange={e => setFormData({ ...formData, intensidadTratador: e.target.value })} className="w-full bg-white border border-pink-200 px-4 py-2.5 rounded-xl text-sm font-medium outline-none focus:border-pink-500 focus:ring-4 focus:ring-pink-500/10" /></div>
